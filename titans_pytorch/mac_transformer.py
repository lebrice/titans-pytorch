    ...
    def forward(
        ...
        if seq.is_cuda and self.use_flex_attn and not disable_flex_attn:
            return self.forward_flex(
                seq, value_residual, flex_attn_fn, output_gating=output_gating
            )
...
class MemoryAsContextTransformer(Module):
    def __init__(
        ...
        *,
        ...
        assert not (
            num_longterm_mem_tokens > 0 and len(neural_memory_layers) == 0
        ), "empty `neural_memory_layers` when longterm memory tokens are present"
        ...
        for layer in layers:
            ...
            if layer in neural_memory_layers:
                assert (
                    has_longterm_mems
                ), "`num_longterm_mem_tokens` must be greater than 0"
    ...
    def sample(
        ...
        if use_cache:
            ...
            longterm_mem_lens = (
                round_up_seq_len // self.segment_len
            ) * self.num_longterm_mem_tokens
    ...
    def forward(
        ...
        batch, seq_len, neural_mem_segment_len, segment_len, num_longterm_mem_tokens = (
            *x.shape,
        ...
        x, inverse_segment = pad_and_segment_with_inverse(
            x, segment_len + num_longterm_mem_tokens
        )
